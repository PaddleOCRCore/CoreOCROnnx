...
- 测试图片：洗发水图
- 每个后端测试循环：50 次的平均时间
- 初始化预热：benchmark 固定执行 2 次，不计入上述测试循环
- 推理线程参数：8；所有选中后端使用同一参数
- 方向分类（cls）：已开启；mostAngle=true
- 其他处理耗时：全流程减去检测、方向分类和文字识别耗时，包含裁剪、旋转、结果组装与排序等开销
- CPU 后端强制 `use_gpu=false`；GPU 后端固定使用设备 0
- 本报告仅测试 OCR，不初始化或测试 YOLO 模型
...
- 内存：64 GB
- 系统架构：64 位 Windows
- GPU：NVIDIA GeForce RTX 5070 Ti（驱动 32.0.15.9636）

## Windows11测试结果

| 后端 | 设备 | 状态 | 检测平均 ms | 方向分类平均 ms | 文字识别平均 ms | 其他处理平均 ms | 全流程平均 ms | 每秒图片数 | 相对最快 |
|---|---:|---|---:|---:|---:|---:|---:|---:|---:|
| ONNX CPU x86 | CPU | 通过 | 25.645 | 55.332 | 87.532 | 1.037 | 169.547 | 5.90 | 8.62x | 
| ONNX CPU x64 | CPU | 通过 | 10.482 | 19.057 | 35.944 | 0.826 | 66.308 | 15.08 | 3.37x | 
| ONNX DirectML x86 | GPU | 通过 | 6.846 | 19.425 | 84.069 | 1.015 | 111.356 | 8.98 | 5.66x | 
| ONNX DirectML x64 | GPU | 通过 | 10.050 | 22.671 | 165.500 | 0.974 | 199.195 | 5.02 | 10.13x |
| OpenVINO CPU x64 | CPU | 通过 | 35.467 | 26.214 | 120.258 | 0.923 | 182.862 | 5.47 | 9.30x | 
| OpenVINO GPU x64 | GPU | 通过 | 19.607 | 20.369 | 59.206 | 0.948 | 100.130 | 9.99 | 5.09x | 
| TensorRT GPU x64 | GPU | 通过 | 3.715 | 3.172 | 11.325 | 1.454 | 19.665 | 50.85 | 1.00x | 

## Ubuntu24.04测试结果

| 后端 | 设备 | 状态 | 检测平均 ms | 方向分类平均 ms | 文字识别平均 ms | 其他处理平均 ms | 全流程平均 ms | 每秒图片数 | 相对最快 |
|---|---|---|---:|---:|---:|---:|---:|---:|---:|
| ONNX CPU x64 | CPU | 通过 | 11.223901 | 15.613500 | 47.821556 | 0.690671 | 75.349628 | 13.271466 | 0.529618 |
| OpenVINO CPU x64 | CPU | 通过 | 24.259921 | 30.524850 | 86.820052 | 0.666809 | 142.271632 | 7.028808 | 1.000000 |
| TensorRT GPU x64 | GPU | 通过 | 3.465908 | 3.236874 | 12.451773 | 1.618670 | 20.773225 | 48.138890 | 0.146011 | 

## 二、运行环境
项目运行环境为VS2022+.net10.0：

核心文件 `PaddleOCROnnx.dll`（Linux 为 `PaddleOCROnnx.so`）是 C++ 动态链接库。其 CPU/GPU 能力取决于所使用的后端运行时包；应用进程的平台和架构必须与运行时包一致。
